...
Si vous êtes un joueur de Minecraft modé, vous allez avoir besoin de vérifier si vous avez été infecté par le
malware fractureiser pour être sur que votre ordinateur et vos données personnels ne courent aucun risque. Vous pouvez faire
...
sur le site d’hébergement de mods CurseForge et sur le hub de plugins dev.craftbukkit.org (à ne pas confondre avec le logiciel Bukkit lui-même). Le profile d'une développeur connue a téléchargé l'un de ces mods pour l'essayer, ce qui a infecté les fichiers de ce ordinateur sans sa connaissance - incluant une copie d'un projet qui aurait pu être publié sur CurseForge comme un fichier légitime, avec le virus inclus.

Cette suite d’événements s'est répété plusieurs fois sur d'autres utilisateurs de CurseForge et dev.craftbukkit.org, infectant des copies de plusieurs plugins et mods populaires. *Il y a des traces de ces fichiers malicieux remontant à mi-Avril.*

Pendant ce temps, le créateur du malware a essayé de "brute force" les comptes de créateurs ayant des mods populaires sur CurseForge. Pour le moment, nous ne sommes pas sur de s'il s'agit d'une brèche causé par le "brute force" ou par l'infection des comptes des créateurs.
...
* S'auto propager à *tout* les fichiers `jar` sur votre machine, affectant probablement les mods
...
* Remplacer les adresses de cryptomonnaies contenu dans le presse papier par des versions alternatives qui semblent appartenir à l'attaquant[^1]
...
**Jusqu'à nouvelle ordre, ayez une précaution extrême sur l'installation de mods Minecraft, sans tenir compte
de la source.** Bien que le serveur de contrôle du logiciel est maintenant hors ligne, **tous les téléchargements provenant de CurseForge ou de Bukkit dans les 2-3 dernières semaines doivent être traité comme potentiellement dangereux**. Certains anti virus ont commencé à ajouter la signature du malware dans leurs bases de données, mais tant que tous n'auront pas mis à jour, restez précautionneux.

*Pour l'instant, nous ne pouvons pas affirmer qu'aucun service d'hébergement de mods est intact*. Agissez avec précaution sans tenir compte de quel site vous utilisez. Il est même possible que les répertoires Maven soit infecté, et ce malware remonte à plusieurs mois.

Pour le moment, de nouvelles infections sont impossible car le serveur de l'attaquant a été fermé, des infections existantes peuvent encore être active.
...
Plusieurs scanners existent qui prennent un fichier de mod et détectent si il contient l'Étape 0 du logiciel malveillant.

* Le [scanner](https://github.com/overwolf/jar-infection-scanner/releases) d'Overwolf
  * Il s'agit de l'option recommandé pour tester votre jeu entier - juste donner le dossier où se trouve vos instances
...
Si ils sont présents, vous êtes possiblement *complètement infecté* et devriez continuer à lire les instructions de ce document.

Beaucoup d'anti virus ont commencé à détecter les fichiers de l'étape 2. Si vous recevez un avertissement que de tel fichier ont été trouvé et supprimé, passez à la section "Je suis infecté·e, que faire ?"
...
* Dans le dossier Local appdata, vous devez vous assurer que votre explorateur de fichier est configurer pour permettre de voir les `Fichiers cachés` et les `Fichiers protégés du Système d'Exploitation`. 
  ...
  * Si vous n'êtes pas sur de comment faire, une vidéo explicative (en anglais) [se trouve ici](https://youtu.be/KLTlTlnXeKs).
...
* Si le dossier `Microsoft Edge` existe, vous êtes infecté. Si c'est le cas, supprimé définitivement ce dossier et son contenu.
  * Si le dossier ne peut pas être supprimer, couper tout les programmes Java via le Manager des Tâches.
...
Premièrement, assurez vous que la méthode que la méthode que vous utilisez pour voir les fichiers est capable de voir les fichiers cachés. La plupart des explorateurs de fichiers graphiques ont le raccourci Ctrl+H pour afficher/cacher les fichiers cachés. Si vous faites, ceci via un invite de commandes, utilisez `ls -A` dans les dossiers correspondant, ou `ls -lha` pour plus d'informations.

Si un seul des fichiers suivant existes, vous êtes infecté. Dans ce cas, supprimez les tous :
* `~/.config/systemd/user/systemd-utility.service`
* `/etc/systemd/system/systemd-utility.service`
* `~/.config/.data/lib.jar`

Ce faisant, si possible, vérifiez votre `journalctl` pour tout les changements que vous pouvez reconnaître. Vous pouvez faire cela via la commande `journalctl -exb` (pour les journaux systèmes) et `journalctl -exb --user` (pour les journaux utilisateurs). Executer les commandes suivantes pour rafraîchir vos services systemd:
```sh
sudo systemctl daemon-reload # Entrez votre mot de passe utilisateurs
systemctl --user daemon-reload 
```

#### Scripts

*Si vous ne savez pas utilisez des scripts PowerShell ou Bash, ceci n'est pas pour vous*  
...
**IMPORTANT**: Nous ne connaissons pas encore l’entièreté de ce que peux réaliser le malware, ni ses intentions, donc une précaution extrême doit être appliqué jusqu'à ce qu'une façon de supprimer tout les symptômes soit trouvée. Tout ce qui est noté ici est seulement *ce que nous savons* - gardez un œil sur les communications de l'équipe pour de possible mise à jour si quoi que ce soit de critique est trouvé.

Si vous avez trouvé des fichiers de l'Étape 2 de fractureiser sur votre ordinateur, il est fortement possible que l'Étape 3 ai été exécuté et ai infecté votre appareil. Votre meilleure option est de considéré que tout ce qui se trouve sur votre ordinateur est *entièrement compromis*. Vous devrez :

* Faire une Back Up de tout ce que vous ne voulez pas perdre sur une clé USB ou une mémoire externe (vous devriez faire ceci souvent dans tout les cas !)
* Utiliser un ordinateur différent, changer vos mots de passes pour tout les services au quel vous étiez connectés sur votre ancien ordinateur (Discord, boite mails, etc...). Si possible utilisez une gestionnaire de mots de passe comme [BitWarden](https://bitwarden.com).
* Si vous n'utilisez pas encore d’Authentification à Double Facteur (l'application Authentificar ou SMS) pour tout les services le proposant, commencez à le faire immédiatement
* Si vous le pouvez, contactez un service professionnel pour effectuer un diagnostique plus détaillé sur votre appareil pour quoi que ce soit de suspicieux, ou pour vous aider à tout effacer et réinstaller l'ordinateur.
...
CurseForge en lui même n'est pas compromis, seulement certains comptes. Ce n'est pas un problème de CurseForge, il se trouvent juste qu'il s'agit de l’endroit où tout s'est produit. CurseForge ont aussi [publié un article](https://support.curseforge.com/en/support/solutions/articles/9000228509-june-2023-infected-mods-detection-tool/) (en anglais) décrivant la situation de leur point de vue et travaillent activement pour déployer des mesures de sécurité supplémentaires.
...
Le fait qu'aucun mod n'ai été infecté est un coup de chance.
...
### Quels Antivirus sont capable d'identifier le malware ?
De nouveaux sont en train d'être ajouter au moment où nous parlons, il est mieux de rester à faire la vérification manuel ci dessus plutôt que de compter sur les AV pour le moment.

### Est ce que je peux aller en multijoueur ?
Oui, vous ne pouvez pas être infecté via un serveur multijoueur si vous ne télécharger pas de mods pour lui.

### Bedrock est-il touché ?
Non, cela n'affecte que la version Java de Minecraft.

### Est ce que des clients alternatifs tel que Lunar ou Badlion sont sûre ?
Voir ci dessous.

### Est ce que Optifine est sûre ? / Est ce que Sodium est sûre ? / Est ce que Iris est sûre ? / Est ce que Create est sûre ? / Est ce que Essential est sûre ? / Est ce que (nom de mod) est sûre ?

**Nous ne pouvons pas entièrement confirmé la sécurité d'aucun mod.**

L'une des fonctions de l'Étape 3 du virus est infecté le plus de `.jar`s qu'il peut trouver sur votre ordinateur. Il peut infecté **tout les .`jar`s**, y compris Minecraft (vanilla/moddé), des mods, des plugins, et des applications Java sans rapport. Donc si votre ordinateur à l'Étape 3 du virus, savoir si le mod que vous venez de télécharger est sûre n'est plus utile - il va devenir infecté.
Détecter et supprimer les Étapes 1 et 2 de fractureiser **en premier**.

Pour le moment, tout les mods infectés **connus** ont été supprimé de CurseForge. CurseForge [ont posté une liste des mods infectés connues](https://support.curseforge.com/en/support/solutions/articles/9000228509-june-2023-infected-mods-detection-tool/) (en anglais, en fin de page) et les ont supprimé du site. Modrinth a scanné 10 mois de mods publiés et n'ont trouvé aucune infection.

Cela *ne garantie pas que des mods infectés aient passés les vérifications*. Pour vérifier si un `jar` donné contient l'Étape 0 de fractureiser, regardez la section [Suis-je infecter ?](#suis-je-infectée) et surtout ayez une précaution extrême en téléchargeant quoi que ce soit lié aux mods pour le moment.

### A quel point l'infection a été répandu ?

CurseForge a indiqué que les fichiers infectés ont été téléchargé près de 6 000 fois pour leur entière période d'infection. Citation de l'annonce de CurseForge sur Discord :
...
### Est ce que fractureiser peut s'échapper des VMs (Machines Virtuels)?
...
Notez que cette sorte d'"évasion presse papier" n'est rien de nouveau et elle est très facile à faire échouer en ne partageant pas son presse papier entre les systèmes hébergeur et hébergé. Utilisez une machine virtuelle plus lourde que le Bac à sable, et désactiver les fonctionnalités partageant le presse papier ("Guest Addons" pour VirtualBox ou l'intégration des services pour Hyper-V).

(De *vrai* techniques d'évasion de machines virtuelles valent des millions de dollars et ne seraient pas utilisé pour des enfants jouant à Minecraft, et nous avons des raisons de croire que l'auteur de ce malware n'est pas un très bon programmeur en premier lieu.)
...
CurseForge ont développé un [détecteur pour les étapes 2 et 3](https://github.com/overwolf/detection-tool) ainsi qu'un [détecteur d'étape 0](https://github.com/overwolf/jar-infection-scanner), ils ont aussi scanné **tout** les mods et plugins publiés à la recherche d'infection d'Étape 1, et ont supprimé tout les cas d'infection **connus**.
...
### Quels adresses puis-je bloquer dans mon firewall/ma table de routage ?

Le code lié à fractureiser a pu être observer se connecter aux URL et adresses suivantes sur un grand nombre de ports :
...
Voici quelques adresses additionnels à bloquer lié à "skyrage" (un ancien virus visant les serveurs Spigot/Paper, il est *très* improbable que skyrage eu été téléchargé sur l'ordinateur de qui que ce soit via ce moyen, mais rien de bon ne viens de ces adresses dans tout les cas):

* `95.214.27.172`
* `connect.skyrage.de`
* `t23e7v6uz8idz87ehugwq.skyrage.de`
* `qw3e1ee12e9hzheu9h1912hew1sh12uw9.skyrage.de`
* *Bloqué tout `skyrage.de` honnêtement*

Cela va sans dire que vous ne devriez en aucun cas visitez ces sites.
...
Les antivirus ne peuvent détecter que les menaces connues, et non les malwares nouveaux.

Utiliser un "bac à sable ": Inclure une sorte de vérification "est ce que ce que contient cette classe est du code 'sûre' ?" avant de charger la classe est un très bon moyen de lancer une jeu de chat contre souris entre les développeurs mal intentionnés et les développeurs des plateformes de mods.

Il est très difficile de bannir du code Java d'utiliser une certaine classe (disons, `URLClassLoader`) car vous pouvez y référer en tant que `Class.forName` (qui a énormément d'utilisations légitimes), et si vous bannissez ou mettez sur liste noire des classes de ce moyen, vous pouvez basiquement trouvez d'*autres* moyens d'utiliser `Class.forName`, et essayer de se débarrasser de ces utilitaires dans une partie sans fin de Tape Taupe.
...
### Serait-il possible d'interdire la possibilité de télécharger du code exécutable aus mods ?
...
* Qu'en est-il d'un fichier contenant un paragraphe en Anglais où il *apparaît* que les phrases avec un nombre pair de mots correspond à un 0, et celle avec un nombre impair à un 1 ? - même s'il s'agit uniquement d'un paragraphe, je peux techniquement l'assembler en un fichier executable, si j'en ai envie.
